...
function Update() {
  ...
  const getUserData = async () => {
    const { status, data } = await apiCall(`users/details/${id}`, "GET", "", { "Content-Type": "application/json" })
    if (!status) { setError(data?.msg || "Failed to fetch user details"); setUserData(null); return; }
  ...
  const handleEdit = async (event) => {
    ...
    const { status, data } = await apiCall(`users/update/${id}`, "PATCH", addUser, { "Content-Type": "application/json" })
    if (!status) { setError(data?.msg || "Failed to update user"); setResponse(""); return; }
    setResponse(data?.msg || "User updated successfully");
    setError("");
    setName("");
    setEmail("");
    setAge(0);
